...
export default function ComparePage() {
  ...
  function ScoreDelta() {
    ...
    if (diff === 0) return <div className="flex items-center gap-1 text-muted-foreground text-sm"><Minus className="w-4 h-4" /> No change</div>
    if (diff > 0) return <div className="flex items-center gap-1 text-emerald-400 text-sm"><TrendingUp className="w-4 h-4" /> +{diff} points improvement</div>
    return <div className="flex items-center gap-1 text-red-400 text-sm"><TrendingDown className="w-4 h-4" /> {diff} points decrease</div>
  }
                  ...
                  >
                    ...
                    {resumes.map(r => (
                      <option key={r.id} value={r.id}>
                        {r.originalName} (v{r.version}) · {r.analyses[0] ? `${r.analyses[0].atsScore}/100` : 'Not analyzed'}
                      </option>
                    ))}
                  </select>
                </div>
                      ...
                      <div key={resume.id} className={cn('rounded-xl border p-6', accent)}>
                        ...
                        {score !== null && colors ? (
                          ...
                          <div className="py-4 text-center">
                            ...
                            <a href={`/upload`} className="text-xs text-primary hover:underline mt-1 block">Analyze now →</a>
                          </div>
                        )}
